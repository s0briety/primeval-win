...
void c_movement::on_create_move(bool a1) {
	...
	if (globals::config::bunnyHop)
		RunBunnyHop();

	compute_buttons();
}

void c_movement::RunBunnyHop()
{
	// Only jump if the player is on the ground and the jump button is held
	if (globals::m_local->get_flags().has(FL_ONGROUND) && globals::m_cur_cmd->m_buttons.has(IN_JUMP))
	{
		// Set the jump button
	...
	{
		// Remove the jump button if not on the ground or the button isn't pressed
