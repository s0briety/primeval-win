...
bool c_chams::on_draw_model(i_model_render* ecx, void* context, const draw_model_state_t& state, const model_render_info_t& info, matrix3x4_t* bones) {
	...
	if (player->get_team() != globals::m_local->get_team() && globals::config::chamsEnemies)
	{
		ImColor color = globals::config::enemyChamsColor;
	...
	else if (player->get_team() == globals::m_local->get_team() && globals::config::chamsTeammates && player != globals::m_local)
	{
		ImColor color = globals::config::teammateChamsColor;
		col_t convertedColor = globals::ImColorToCol_T(color);
	...
	{
		ImColor color = globals::config::selfChamsColor;
		col_t convertedColor = globals::ImColorToCol_T(color);
