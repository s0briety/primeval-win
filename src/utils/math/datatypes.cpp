...
bool vec3_t::to_screen(vec2_t& value) const {
	...
	value.x = ImGui::GetIO().DisplaySize.x * 0.5f + (value.x * ImGui::GetIO().DisplaySize.x) * 0.5f;
	value.y = ImGui::GetIO().DisplaySize.y * 0.5f - (value.y * ImGui::GetIO().DisplaySize.y) * 0.5f;
